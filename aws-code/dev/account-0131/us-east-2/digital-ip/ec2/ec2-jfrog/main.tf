...
data "terraform_remote_state" "jfrog-sg" {
  backend = "s3"
  config = {
    # Replace this with your bucket name!
    bucket = "mt-tfstate-${var.env_name}-${var.aws_account_short_id}"
    key    = "terraform-code/${var.env_name}/${var.local_dir}/${var.aws_region}/${var.project_name}/security-group/sg-jfrog/terraform.tfstate"
    region = "us-east-2"
  }
}
...
module "ec2" {
  ...
  vpc_security_group_ids      = [data.terraform_remote_state.jfrog-sg.outputs.id]
  associate_public_ip_address = true
  user_data = templatefile("${path.cwd}/install_jfrog.sh", {})
 ...
 tags = {
    ...
    "Name" = "jfrog-for-${var.project_name}"
  }
}