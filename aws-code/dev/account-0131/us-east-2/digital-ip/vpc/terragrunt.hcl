...
terraform {
  source = "/mnt/c/Users/riyajk/Documents/mt-devops/terraform-module/vpc"
}
...
inputs = {
  ...
  # type: list(string)
  database_subnets = [for k,v in dependency.aws-data.outputs.available_aws_availability_zones_names: cidrsubnet("10.10.0.0/16", 5, k+8)]
  ...
  # type: list(string)
  private_subnets = [for k,v in dependency.aws-data.outputs.available_aws_availability_zones_names: cidrsubnet("10.10.0.0/16",5,k)]

  # A list of public subnets inside the VPC
  # type: list(string)
  public_subnets = [for k,v in dependency.aws-data.outputs.available_aws_availability_zones_names: cidrsubnet("10.10.0.0/16", 5, k+12)]
